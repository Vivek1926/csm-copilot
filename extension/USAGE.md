...
Meet call, transcribes it (Deepgram cloud by default, or fully-local Whisper
— switchable in Settings), detects when the customer asks a technical
question, and shows each question as a card in a side panel.
...
Settings in the panel (expand **Settings**) control STT quality:

- **STT engine** — **Deepgram cloud** (default): most accurate, starts
  instantly (no model download), needs internet; utterance audio is sent to
  Deepgram's API. **Whisper local**: fully private/offline alternative —
  audio never leaves the machine.
- **Deepgram API key** — a built-in demo key is used if empty; paste your
  own to override.
- **Whisper model** (local engine only) — `whisper-small` (accurate) or
  `whisper-base` (fast). A change takes effect on the next capture start.
- **Domain terms** — product and brand names speech models can't spell
  ("Seclore" comes out as "secular", "cyclore", "the clue"…). List them
  comma-separated (e.g. `Seclore, Seclore Online, policy server`). They are
  used twice: as **Deepgram keyword boosts** (recognized correctly at
  transcription time) and as fuzzy post-corrections for anything still
  misheard. Common Seclore terms are built in; add your own here.
...
- **Your own questions may fire cards.** Speaker separation isn't in the
  ...
  dismiss it.
- **Privacy:** with the default **Deepgram** engine, utterance audio is sent
  to Deepgram's API for transcription. Switch the STT engine to **Whisper
  local** in Settings and no audio leaves your machine at all. Question
  *text* (plus a few seconds of surrounding transcript) goes to the Astra
  LLM for refinement and answers.
