...
export function parseAnswer(md) {
  ...
  let para = [];
  let list = null;
  let tableRows = null; // raw cell-text rows while inside a | table |
  ...
  const flushTable = () => {
    if (!tableRows) return;
    // A |---|:---:| row after the first marks it as the header row.
    let header = null;
    let rows = tableRows;
    if (rows.length >= 2 && rows[1].every((c) => /^[\s\-:]+$/.test(c) && c.includes('-'))) {
      header = rows[0];
      rows = rows.slice(2);
    }
    const toInlines = (cells) => cells.map((c) => parseInline(c.trim(), sources));
    blocks.push({
      type: 'table',
      header: header ? toInlines(header) : null,
      rows: rows.map(toInlines),
    });
    tableRows = null;
  };
  ...
  for (const rawLine of String(md || '').split(/\r?\n/)) {
    ...
    // Markdown table row: "| a | b |" (allow a missing trailing pipe).
...
export function renderAnswerInto(container, md) {
  ...
  for (const block of blocks) {
    if (block.type === 'heading') {
      ...
      // Wrapped so wide tables scroll inside the card instead of overflowing.
