...
// only chrome.runtime is available here). Holds Stage-2 creds, STT model
...
function applyConfig(cfg) {
  ...
  const extra = String(config.domainTerms || '')
    ...
    .map((t) => t.trim())
    .filter(Boolean);
  correctTerms = createCorrector([...DEFAULT_TERMS, ...extra]);
}
...
function onWhisperResult({ id, text, error, sttMs }) {
